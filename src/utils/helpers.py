...
def setup_simulation(cfg):
    ...
    base_dir = Path.cwd()
    datapath = str(
        base_dir / cfg["paths"]["dataset_dir"] / cfg["paths"]["ply_filename"]
    )
    output_path = str(
        base_dir / cfg["paths"]["result_dir"] / cfg["paths"]["camera_mcap_filename"]
    )
    ...
    #fix drift
    motion_dir[2] = motion_dir[2] + 0.02  # eliminate drift, need a better solution
    ...
    K = np.array([[FL, 0, W / 2.0], [0, FL, H / 2.0], [0, 0, 1]])
